...
const KEY_DEFAULTS: Record<string, string> = {
  ...
  sessions: 'dir: sessions/ in the config directory; resume: true — the chat continues the latest session on start (a restart or an update loses nothing); keep: 50 sessions. In the chat, /sessions (or Ctrl+S, from any screen; config set keys.sessions <key> moves it) lists every saved session newest first with its title, and typing filters by the title and the conversation\'s words — Enter opens one, Ctrl+N starts a new one, Ctrl+R renames, Ctrl+X deletes after a y/n (a session open in another flow-assist process is refused); /resume lists them by number and /resume <n> opens one; /new starts a new session and keeps the current one open, so a restart before anything is said continues it; /clear starts a new session and marks the old one done; /title <text> renames the current session (it is named by the first line the person wrote)',
