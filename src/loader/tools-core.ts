...
const KEY_DEFAULTS: Record<string, string> = {
  ...
  shell: `timeoutMs: ${SHELL_DEFAULTS.timeoutMs} (the whole process group is killed after it), maxChars: ${SHELL_DEFAULTS.maxChars} (the END of the output is kept), roots: [] (no roots: commands start in the app's own directory and cd anywhere). Two ways to run a command: the person types !command in the chat (e.g. !bun test) — it runs in the first shell.roots directory (else the app's own), the directory is remembered between commands like a terminal's (cd moves it, only within the roots; variables are not kept; /clear goes back to the first root), Esc stops it, and the output joins the conversation without spending a model turn; and the model's run_command tool, which asks the person y/n before every command and is never run by a background task. config set ai.disabledTools ["shell"] turns run_command off (! stays)`,
